...
export function AddIncomeModal({ visible, onClose, editing }: Props) {
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const qc = useQueryClient();
  const { data: categories = [] } = useGetCategories({ type: "income" });
  ...
  const updateMutation = useUpdateIncome({
    mutation: {
      ...
      onError: (err: any) => {
        const msg = err?.response?.data?.message ?? "Could not update income.";
        Alert.alert("Error", msg);
      },
    },
  });
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={[styles.header, { paddingTop: insets.top + 16, borderBottomColor: colors.border }]}>
          ...
          <Text style={[styles.title, { color: colors.text }]}>{editing ? "Edit Income" : "Add Income"}</Text>
          <Pressable onPress={handleSave} disabled={isPending} style={({ pressed }) => [pressed && { opacity: 0.6 }]}>
            {isPending ? (
              ...
              <Text style={[styles.save, { color: colors.income }]}>Save</Text>
            )}
          </Pressable>
        </View>
              ...
              {categories.map((c) => (
                <Pressable
                  ...
                  style={[
                    styles.chip,
                    {
                      backgroundColor: selectedCat === c.id ? colors.income + "25" : colors.card,
                      borderColor: selectedCat === c.id ? colors.income : colors.border,
                    },
                  ]}
                >
                  <Text style={[styles.chipText, { color: selectedCat === c.id ? colors.income : colors.text }]}>
                    {c.name}
                  </Text>
                </Pressable>
              ))}
              ...
              {RECURRENCE_OPTIONS.map((opt) => (
                <Pressable
                  ...
                  style={[
                    styles.chip,
                    {
                      backgroundColor: recurrenceType === opt.key ? colors.income + "25" : colors.card,
                      borderColor: recurrenceType === opt.key ? colors.income : colors.border,
                    },
                  ]}
                >
                  <Text style={[styles.chipText, { color: recurrenceType === opt.key ? colors.income : colors.text }]}>
                    {opt.label}
                  </Text>
                </Pressable>
              ))}
